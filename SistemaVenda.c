...
void imprimir_relatorio(){    
 FILE *arquivo = fopen("relatorio.txt", "w");
    ...
    fclose(arquivo);
    printf("Relatorio salvo com sucesso no arquivo 'relatorio.txt'.\n");
}
    ...
    for (i = 0; i < QTD_PRODUTOS; i++) {
        if (strcmp(produtos[i], nome) == 0) {
            ...
            while(getchar() != '\n');
                ...
                if (total_venda < MAX_VENDAS) {
                strcpy(vendas[total_venda].nome, nome);
                vendas[total_venda].quantidade = quantidade;
                total_venda++;
}
...
int main(){
    ...
    do {
        ...
        printf("9. Imprimir relatorio de vendas\n");
        printf("0. Sair\n");
        printf("Opcao: ");
        scanf("%d", &opcao);
        while(getchar() != '\n');
